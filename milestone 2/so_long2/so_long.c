...
/*   Updated: 2025/09/17 20:52:38 by olacerda         ###   ########.fr       */
/*                                                                            */
/******************************************************************************/
...
void	sheet_to_image_convertor(t_sheet *src, t_image *dst, int sprite_column, int sprite_line)
{
	int	bytes_per_line;
	int	img_line;
	int src_idx;
	...
	src_idx = (dst->sizeline * (sprite_column - 1)) + (src->sizeline * ((sprite_line - 1) * dst->tall));
	img_line = 0;
	count = 0;
	dst_idx = 0;
...
t_all_images	*all_images_initiator(void *mlx)
{
	...
	sheet_to_image_convertor(images->grass_wall_sheet, images->wall, 1, 1);
	sheet_to_image_convertor(images->grass_wall_sheet, images->grass, 2, 1);
	color_image(images->player, 0);
	color_image(images->collectable, 501000);
	color_image(images->exit, 16777215);
	sheet_to_image_convertor(images->letters_sheet, images->e, 5, 1);
	sheet_to_image_convertor(images->letters_sheet, images->x, 2, 1);
	sheet_to_image_convertor(images->letters_sheet, images->i, 2, 1);
	sheet_to_image_convertor(images->letters_sheet, images->t, 2, 1);
